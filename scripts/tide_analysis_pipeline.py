...
def plot_tide_analysis(control_file, edited_file, output_dir, gene_name):
    ...
    ax1.set_title(f'{gene_name} - Control Sample (WT) Chromatogram', fontsize=14, fontweight='bold')
    ...
    ax2.set_title(f'{gene_name} - Edited Sample Chromatogram', fontsize=14, fontweight='bold')
    ...
    ax3.set_title(f'{gene_name} - Overlay: Control (solid) vs Edited (dashed)', fontsize=14, fontweight='bold')
    ...
    for base, color in colors.items():
        if focus_end <= len(control_traces[base]) and focus_end <= len(edited_traces[base]):
    ...
    # Save the plot
    output_file = os.path.join(output_dir, f"tide_analysis_{gene_name}.png")
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"TIDE analysis plot saved to: {output_file}")
...
def process_gene_folder(gene_folder, gene_name):
    ...
    if not all(os.path.exists(f) for f in [control_file, edited_file, grna_file]):
        ...
        print(f"  control.ab1: {os.path.exists(control_file)}")
        print(f"  edited.ab1: {os.path.exists(edited_file)}")
        print(f"  grna.txt: {os.path.exists(grna_file)}")
        return
    ...
    try:
        divergence_point = plot_tide_analysis(control_file, edited_file, gene_folder, gene_name)
        
        # Ask for mRNA sequence if user wants recommendations
        print(f"\nWould you like to generate primer recommendations for {gene_name}?")
        user_input = input("Enter 'y' to input mRNA sequence, or press Enter to skip: ").strip().lower()
        
        if user_input == 'y':
            print(f"\nPlease paste the full {gene_name} mRNA sequence (press Enter twice when done):")
            mrna_lines = []
            while True:
                line = input()
                if line:
                    mrna_lines.append(line.strip())
                else:
                    if mrna_lines:  # Only break if we have some input
                        break
            
            mrna_seq = ''.join(mrna_lines).upper().replace('\n', '').replace(' ', '')
            
            # Generate recommendations
            recommendations = analyze_grna_in_mrna(mrna_seq, grna_sequences, gene_name)
            
            # Save recommendations
            rec_file = os.path.join(gene_folder, "recommendations.txt")
            with open(rec_file, 'w') as f:
                f.write(recommendations)
            
            print(f"\nPrimer recommendations saved to: {rec_file}")
...
def main():
    ...
    print("TIDE Analysis Pipeline")
    print("=" * 60)
    
    # Look for gene folders in workdir
    workdir = "workdir"
    gene_folders = []
    
    for item in os.listdir(workdir):
        item_path = os.path.join(workdir, item)
        if os.path.isdir(item_path) and item in ['vmat1', 'vmat2', 'ddc']:
            gene_folders.append((item_path, item))
    
    if not gene_folders:
        print("No gene folders found in workdir!")
        print("Please create folders named 'vmat1', 'vmat2', or 'ddc' in the workdir")
        return
    ...
    for folder_path, gene_name in gene_folders:
        process_gene_folder(folder_path, gene_name)
